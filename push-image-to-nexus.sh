...
CAKE3_SDK_NAME=`jq '.cake3_sdk_name' version.json | tr -d '"'`
CAKE3_BRANCH_NAME=`jq '.cake3_sdk_branch' version.json | tr -d '"'` 

echo "SDK: ${CAKE3_SDK_NAME}  , branch: ${CAKE3_BRANCH_NAME}  ,  version: ${PACKAGE_VERSION}"

docker build -t  ${IMAGE_NAME}:${SHORT_COMMIT} --build-arg PackageName=${PACKAGE_NAME} --build-arg PackageVersion=${PACKAGE_VERSION} --build-arg NexusUser=${NEXUS_USER} --build-arg NexusPassword=${NEXUS_USER_PWD} --build-arg Cake3SDK=${CAKE3_SDK_NAME} --build-arg Cake3Branch=${CAKE3_BRANCH_NAME} .
sleep 3
docker image ls 
